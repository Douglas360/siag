...
const ListOfficialDocumentJs = () => {
    ...
    useEffect(() => {
        async function loadUserReadDocuments() {
            // Chame a função que obtém a lista de documentos lidos pelo usuário logado
    ...
    useEffect(() => {
        async function loadOfficialDocumentRead() {
            try {
                // Chame a função que obtém a lista da quantidade de usuarios que leram o documento
    ...
    const rows = officialDocument?.map((row) => {
        const isRead = userReadDocuments.some((doc) => doc.id_doc_oficial === row.id_doc_oficial); // Verificar se o documento foi lido pelo usuário
        return {
            ...
            documento: (
                ...
                >
                    {isRead ?
                        getFileIcon(row.arquivo, '#7c7c7c')
                        :
                        getFileIcon(row.arquivo, '#0051ff')}
                </span>
                ...
                <div className="d-flex justify-center">
                    <div className="ml-2">
                        ...
                        >
                            {
                                leitura?.map((doc) => {
                                    if (doc.id_doc_oficial === row.id_doc_oficial) {
                                        return (
                                            <div key={doc.id_doc_oficial}>
                                                {doc.qnt_user_read} / {doc.total_users}
                                            </div>
                                        )
                                    }
                                }
